...
const notificationUrl = envString('NOTIFICATION_SERVICE_URL', 'http://localhost:3007');
...
app.post(
  ...
  asyncHandler(async (req: AuthenticatedRequest, res) => {
    ...
    const userTrainingPlan = await repository.save(
      repository.create({
        userId: user.id,
        planId,
        startDate,
    ...
    try {
      await serviceRequest(notificationUrl, '/api/internal/notifications', {
        method: 'POST',
        body: {
          userId: user.id,
          type: 'training_plan_enrolled',
          title: 'Вы записались на тренировочный план',
          message: `План «${plan.title}» добавлен в ваш список.`,
        },
      });
    } catch (error) {
      notificationCreated = false;
      console.warn('Notification service is unavailable:', error);
    }

    res.status(201).json({ userTrainingPlan, notificationCreated });
  }),
);
...
void startService(app, plansDataSource, port, 'training-plan-service').catch((error) => {
  console.error(error);
  process.exit(1);
});
