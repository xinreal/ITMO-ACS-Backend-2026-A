...
import { IntegrationEvent, rabbitMqStatus, startRabbitConsumer } from '../shared/rabbitmq';
...
type TrainingPlanEnrolledPayload = {
  userId: number;
  planId: number;
...
function startTrainingPlanEnrollmentConsumer(): void {
  startRabbitConsumer({
    ...
    onEvent: async (event: IntegrationEvent) => {
      if (event.type !== 'training-plan.enrolled.v1') {
        throw new Error(`Unsupported event type: ${event.type}`);
      }

      const payload = event.payload as TrainingPlanEnrolledPayload;
      if (!Number.isInteger(payload.userId) || !payload.planTitle) {
        throw new Error('training-plan.enrolled payload is invalid');
      }

      const repository = notificationDataSource.getRepository(Notification);
      const alreadyProcessed = await repository.findOneBy({ eventId: event.id });
      ...
      await repository.save(
        repository.create({
          ...
          type: 'training_plan_enrolled',
          title: 'Вы записались на тренировочный план',
          message: `План «${payload.planTitle}» добавлен в ваш список.`,
          status: 'created',
          sentAt: new Date(),
        }),
      );
...
app.get('/health/rabbitmq', (_req, res) => {
  res.json({ service: 'notification-service', rabbitmq: rabbitMqStatus() });
});
...
void startService(app, notificationDataSource, port, 'notification-service', () => {
  startTrainingPlanEnrollmentConsumer();
}).catch((error) => {
  console.error(error);
  process.exit(1);
});
